...
def create_wallet(wallet: CreateWalletRequest):
    ...
    try:
        if wallets_repository.is_wallet_exist(db, wallet.name):
            raise HTTPException(status_code=404, detail=f"Wallet {wallet.name} already exists")

        wallet = wallets_repository.create_wallet(db, wallet.name, wallet.initial_balance)
        db.commit()

        return {"message": f"Wallet {wallet.name} was created ", "wallet": wallet.name, "balance": wallet.balance}
    finally:
        db.close()
